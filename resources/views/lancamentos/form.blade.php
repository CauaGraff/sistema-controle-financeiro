...
<div id="parcelasFields" style="display: none;">
    ...
    <div class="row mb-3">
        <label for="qtdParcelas" class="col-sm-2 col-form-label" id="labelDataVenc">Quantidade
            Parcelas</label>
        <div class="col-sm-10">
            <input type="number" class="form-control @error('valorEntrada')is-invalid @enderror" id="qtdParcelas"
    ...
    <div class="col-sm-10">
        <select class="form-select @error('fornecedor_cliente_id')is-invalid @enderror" id="fornecedor_cliente"
            ...
            @foreach ($clientes as $cliente)
                <option value="{{ $cliente->id }}" {{ old('fornecedor_cliente_id') == $fornecedor->id ? 'selected' : '' }}>
                    {{ $cliente->nome }}
                </option>
            @endforeach
        </select>
